...
# 定义源文件夹和目标压缩包文件夹路径
source_folder_base = r"D:\GITHOME\maa\插曲&别传\别传"
destination_folder = r"D:\GITHOME\maa\插曲&别传\别传\压缩包"

# 遍历需要压缩的文件夹并进行压缩
for folder_name in os.listdir(source_folder_base):
    if folder_name.startswith("别传"):
        # 构造源文件夹和目标压缩包文件路径
...
shutil.make_archive(os.path.join(destination_folder, "别传"), 'zip', destination_folder)
file_path = r"D:\GITHOME\maa\【下载看这里】合集下载\别传.zip"
if os.path.exists(file_path):
    os.remove(file_path)
shutil.move(os.path.join(destination_folder, "别传.zip"), r"D:\GITHOME\maa\【下载看这里】合集下载")
